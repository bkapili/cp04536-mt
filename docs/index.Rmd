...
cosmos <- read.csv(file = "../data/CP04618_follow_up_20220920.csv")
meta <- read.csv(file = "../data/CP04618_follow_up_metadata.csv")
```
...
# Define sample order
df <- df %>%
  mutate(Plot_label = factor(df$Plot_label, levels = c("EB_3866 (saliva)",
                                                       "EB_3846 (CapScan_(SI))",
                                                       "EB_3847 (CapScan_(SI))",
                                                       "EB_3848 (CapScan_(SI))")))
...
# Set column order
col_order <- c("EB_3866 (saliva)", "EB_3846 (CapScan_(SI))", "EB_3847 (CapScan_(SI))", "EB_3848 (CapScan_(SI))",
...
x <- x[match(row_order, rownames(x)), match(col_order, colnames(x))]
...
pHeat
```
**Figure 2:** Heat map showing relative abundance of microbes in each sample (N = 282 total identified). Each row corresponds to a unique microbe and each column corresponds to an individual sample. Note the relative abundance colorscale is in log units.
...
* *Haemophilus parainfluenza*:
  * An opportunistic pathogen often resistant to B-lactam antibiotics and associated with numerous types of infections (*e.g.*, respiratory tract infections, meningitis, endocarditis, bone and joint infections) (see Introduction from González-Díaz *et al*., 2019)
  * Composed ~30-50% of microbial community in proximal CapScan samples
<br><br>
...
González-Díaz, A., Tubau, F., Pinto, M., Sierra, Y., Cubero, M., Càmara, J., *et al*. (2019) Identification of polysaccharide capsules among extensively drug-resistant genitourinary *Haemophilus parainfluenzae* isolates. *Sci Rep* **9:** 1–8.